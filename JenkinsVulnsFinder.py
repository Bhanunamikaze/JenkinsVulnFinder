...
def detect_auth_type(base_url, auth):
    auth_url = f"{base_url}/user/api/json"
    response = requests.get(auth_url, auth=auth)
    if response.status_code == 403:
        return "Matrix-based Security"
    elif response.status_code == 200:
        return "RBAC or OAuth"
    return "Unknown"

def get_jenkins_version(base_url, auth):
...
def check_plugins(base_url, auth):
    ...
    response = requests.get(plugins_url, auth=auth)
...
def check_stored_credentials(base_url, auth):
    creds_url = f"{base_url}/credentials/store/system/domain/_/api/json?depth=1"
    response = requests.get(creds_url, auth=auth)
    if response.status_code == 200:
...
def check_unauthenticated_endpoints(base_url):
    common_endpoints = [
        "/", "/login", "/signup", "/script", "/api/json", "/computer/api/json",
...
def dir_search(base_url, paths_file, auth=None):
    accessible_paths = []
    try:
        with open(paths_file, "r") as file:
            ...
            for path in paths:
                ...
                # Pass 'auth' only if it's not None (i.e., if credentials are provided)
...
def analyze_jenkins(base_url, auth):
    results = {
        "URL": base_url,
        "Version": get_jenkins_version(base_url, auth),
        "Auth Type": detect_auth_type(base_url, auth),
        "Anonymous Access": check_anonymous_access(base_url),
        "CSRF Protection": check_csrf_protection(base_url, auth),
        "Job Visibility": len(check_job_visibility(base_url, auth)),
        "Script Console": check_script_console(base_url, auth),
        "Node Details": len(check_nodes(base_url, auth)),
        "Uses HTTP": check_http_usage(base_url),
        "Stored Credentials": check_stored_credentials(base_url, auth),
        "Log Access": check_log_access(base_url, auth),
        "Audit Logs Enabled": check_audit_logs(base_url, auth),
        "API Access": check_api_access(base_url, auth),
        "CLI Access": check_cli_access(base_url, auth),
    ...
    try:
        node_data = check_nodes(base_url, auth)
    ...
    jobs = check_job_visibility(base_url, auth)
    if len(jobs) > 0:
    ...
    creds_details = check_stored_credentials(base_url, auth)
    if creds_details:
...
def main():
    parser = argparse.ArgumentParser(
        description="""Jenkins Vulnerability Scanner - A tool to scan Jenkins servers for common security issues.
            ...
            4. Run Scanner with Directory/Path Search - Takes wordlist file as input
...
""", formatter_class=argparse.RawTextHelpFormatter
    ...
    # Ensure either --cred or --nocred is passed along with --url or --urls
    if not ((args.nocred or args.cred) and (args.url or args.urls)):
        parser.print_help()
        print("\nERROR: Either --cred or --nocred must be specified, and either --url or --urls must be provided.")
        return
    ...
    if args.nocred:
        ...
        print("Either --cred or --nocred must be specified.")
        return
    ...
    for url in urls:
        ...
        try:
            result = analyze_jenkins(url, auth)

            # Fetch plugin details for CSV
            plugins = check_plugins(url, auth)
            ...
            if args.dirb:
                accessible_paths  = dir_search(url,args.dirb, auth)
                if accessible_paths: 
